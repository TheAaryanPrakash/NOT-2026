...
const SharedFlashcard = () => {
  ...
  if (isError || !flashcard) {
    return (
      <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center gap-4 text-center px-5">
  ...
  return (
    <div className="min-h-screen bg-slate-100 pb-10">
      <div className="max-w-screen-md mx-auto px-5 pt-10">
        <div className="flex items-center gap-5 mb-10 bg-white p-5 rounded-md shadow-sm">
          <img
          ...
          />
          <div>
            <h1 className="text-2xl font-bold capitalize">{flashcard.name}</h1>
        ...
        <ul className="flex flex-col gap-4">
          {flashcard.flashcard_terms.map(({ term, definition, image_url }, index) => (
            <li key={index} className="bg-white p-5 rounded-md shadow-sm">
              {image_url && (
                <img
                  src={image_url}
                  alt={term}
